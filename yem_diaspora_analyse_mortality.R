...
### Estimating older age survival - exposure of interest: period (pre-war, war and pandemic)
  ...
  ## Sensitivity analysis: simulation varying time to departure from Yemen among people who didn't die in Yemen
    ...
    # Define different ranges for the proportion of time that people spent in the analysis period before leaving
    sens_ranges <- list(c(1, 1), c(0.60, 1), c(0.40, 0.80), c(0.20, 0.60), c(0.00, 0.40) )
    names(sens_ranges) <- c("baseline", "0.60 to 1.00", "0.40 to 0.80", "0.20 to 0.60", "0.00 to 0.40")
    
    # Dataset of individuals and their age at exit from the cohort (age at entry = 0)
    df_id <- by(df, df$id, function(x) {return(c(unique(x[, "id"]), max(x[, "age_end"]) ) )})
    df_id <- as.data.frame(do.call(rbind, df_id) )
    colnames(df_id) <- c("id", "age_end")
    df_id$age_end <- as.numeric(df_id$age_end)
    ...
    # For each range...
    for (range_k in names(sens_ranges) ) {
      
      # Define range to be explored
      sens_range <- sens_ranges[[range_k]]
      
      # Prepare data for simulation
        # generate n_sim randomly censored datasets of exit age based on a uniform distribution of relative departure times
        df_sim <- replicate(n_sim, sapply(df_id$age_end, function (x) {x * runif(1, min = sens_range[1], max = sens_range[2]) } ) )
        df_sim <- cbind(df_id$id, as.data.frame(df_sim) )
        colnames(df_sim) <- c("id", paste("age_end_sim", 1:n_sim, sep = ""))
        
        # dataset to be simulated
        df_sim <- merge(df, df_sim, by = "id", all.x = TRUE)
        
        # apply correction: where the person died inside Yemen, we know that to be the exit date
        x1 <- unique(df[which(df$died_yemen == 1), "id"])
        for (i in 1:nrow(df_sim)) {
          if (df_sim[i, "id"] %in% x1) df_sim[i, grep("age_end_sim", colnames(df_sim))] <- df_sim[i, "age_end"]
        }
        ...
        out <- data.frame(matrix(NA, nrow = n_sim * 3, ncol = 4))
        colnames(out) <- c("range", "period", "hr", "p")
        out$range <- range_k
        out$period <- c(rep("war", n_sim), rep("pandemic", n_sim), rep("war + pandemic", n_sim))
        
      # Run simulation using models above
      for (i in 1:n_sim) {
        # simulate age column
        age_sim <- paste("age_end_sim", i, sep = "")
        
        # update dataset with simulated ages of exit
        df_i <- df_sim[, c(cols_sim, age_sim)]
        
        # exclude periods beyond simulated age of exit
        df_i <- df_i[which(df_i$age_start < df_i[, age_sim]), ]
        
        # in remaining records, replace maximum exit age with simulated value
        x1 <- by(df_i, df_i$id, function(x) {return(cbind(x[, "id"], x[, "age_end"], (x[, "age_end"] == max(x[, "age_end"])) ) )} )
        x1 <- as.data.frame(do.call(rbind, x1))
        colnames(x1) <- c("id", "age_end", "is_max")
        df_i <- merge(df_i, x1, by = c("id", "age_end"), all.x = TRUE)
        df_i[, "age_end"] <- ifelse(df_i$is_max, df_i[, age_sim], df_i$age_end)
        
        # run Cox model for both periods
        fit1 <- coxph(Surv(age_start, age_end, died_yemen) ~ birth_cohort + gender + period , data = df_i)
        fit2 <- coxph(Surv(age_start, age_end, died_yemen) ~ birth_cohort + gender + period_simple , data = df_i)
      ...
      # Add output for this range to the overall output
      out_all <- rbind(out_all, out)
      
    }    
    
    # Compute output means
    sim_means <- aggregate(out_all[, c("hr", "p")], by = out_all[, c("range", "period")], FUN = mean)
    plot_labels <- subset(sim_means, period == "war + pandemic")[, c("range", "hr", "p")]
    plot_labels$hr <- round(plot_labels$hr, digits = 2)
    plot_labels$p <- round(plot_labels$p, digits = 3)
    plot_labels$label <- paste("HR =", plot_labels$hr, "(p =", plot_labels$p, ")")
    
    # Visualise simulation outputs (omit pandemic and war periods separately)   
    plot <- ggplot(data = subset(out_all, period == "war + pandemic"), aes(x = hr, y = p, colour = range, size = range)) +
      geom_point(alpha = 0.5) +
      facet_wrap(. ~ range, scales = "free") +
      scale_x_continuous("hazard ratio") +
      scale_y_continuous("p-value") +
      theme_bw() +
      scale_colour_manual(values = palette_cb[c(4,7,6,8,1)]) +
      scale_size_manual(values = c(1,1,1,1,5)) +
      theme(legend.position = "none", axis.title = element_text(colour = "grey20")) +
      geom_text(aes(-Inf, -Inf, label = label), data = plot_labels, size = 3, hjust = -0.1, vjust = -1)
      
    plot
    ggsave("out_survival_sens.png", dpi = "print", unit = "cm", width = 20, height = 15)
